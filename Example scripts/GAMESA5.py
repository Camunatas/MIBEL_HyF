...
def MD(d, SOCiMD, ESS_Energy, ESS_Power, ESS_Efficiency, ESS_Cost):
    ...
    #Objective Function 
    model.obj = Objective(
        expr = sum(((P[t1]*(model.ESS_D[t1] + model.PV_P[t1] - model.Purchase_ESS[t1])) -
                    ((ESS_Cost*(model.ESS_C[t1] + model.ESS_D[t1]))) for t1 in model.time)),
        sense = maximize)
...
def ID(Schedule_DM, ESS_DM, H_ID, FinalH_ID, h, d, SOCi, ESS_Energy, ESS_Power, ESS_Efficiency, ESS_Cost, Deviation_Cost):
    ...
    def c15_rule(model, t1):
        return model.Deviation_P1[t1] == (Schedule_P_DM[t1] - (((model.PV_P1[t1] + model.ESS_D1[t1]) - model.Purchase_ESS1[t1]) +
                                                               model.Purchase_Grid[t1]))
    model.c15 = Constraint( model.time, rule=c15_rule )

    def c16_rule(model, t1):
        return model.Deviation_P2[t1] == ((((model.PV_P1[t1] + model.ESS_D1[t1]) - model.Purchase_ESS1[t1]) + model.Purchase_Grid[t1]) -
                                          Schedule_P_DM[t1])
    ...
    #Objective Function
    model.obj = Objective(
        expr = sum(((P[t1]*(((model.ESS_D1[t1] + model.ESS_D2[t1] + model.PV_P1[t1] + model.PV_P2[t1]) -
                             (model.Purchase_ESS1[t1] + model.Purchase_ESS2[t1])) - Schedule_P_DM[t1])) -
                    ((ESS_Cost*((model.ESS_C[t1] + model.ESS_D1[t1] + model.ESS_D2[t1]) - ESS_P_DM[t1])) +
                     (Deviation_Cost*(P[t1]*model.Deviation_P[t1])) + (P[t1]*model.Purchase_Grid[t1])) for t1 in model.time)),
        sense = maximize)
...
def TR(Schedule_ID, H_RT, FinalH_RT, h, d, SOCi, ESS_Energy, ESS_Power, ESS_Efficiency, ESS_Cost, Deviation_Cost):
    ...
    #Objective Function
    model.obj = Objective(
        expr = sum(((P[t1]*(((model.ESS_D[t1] + model.PV_P[t1]) - (model.Purchase_ESS[t1])) - Schedule_P_ID[t1])) -
                    ((ESS_Cost*(model.ESS_C[t1] + model.ESS_D[t1])) + (Deviation_Cost*(P[t1]*model.Deviation_P[t1])) +
                     (P[t1]*model.Purchase_Grid[t1])) for t1 in model.time)),
        sense = maximize)
...
plt.plot(range(D*H), PV_RT, label='PV_P') #PV Power Use
